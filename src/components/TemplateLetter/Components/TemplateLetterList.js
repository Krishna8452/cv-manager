...
export const TemplateLetterList = () => {
    ...
    useEffect(() => {
      ...
      const fetchData = async () => {
        try {
          const response = await fetch("http://localhost:3031/templateLetters");
    ...
    const deleteTemplateLetter = (id)=>{
        axios.delete(`http://localhost:3031/templateLetters/${id}`);
 ...
 console.log(templateLetterList,'dddd')
  return (
   ...
   <TableContainer sx={{ display: "flex", marginTop: 1 }} component={Paper}>
        ...
        <>
          <h1 style={{ marginLeft: "34%" }}>Template Letter List</h1>
          ...
          <Box sx={{ marginLeft: "15px", display: "flex" }}>
            ...
            <IconButton onClick={() => navigate("/templateLetter/create")}>
              <Icon
    ...
    <Table>
        ...
        return <>
        <TableBody>
            <TableCell size='small'>{list.name}</TableCell>
            <TableCell size='small'>{list.subject}</TableCell>
            <TableCell>
                <ButtonGroup>
                <Button
